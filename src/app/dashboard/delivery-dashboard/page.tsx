...
import { checkForNewOrderToPickup, markOrderAsDelivered, markOrderAsOutForDelivery } from "@/actions/diliveryAgent"
...
//   const [deliveredOrders, setDeliveredOrders] = useState<Order[]>([])
  ...
  function getOrdersWithOutForDeliveryStatus(orders: Order[]) {
    return orders.filter(order => 
      order.status.trim().toLowerCase() === "order out for delivery".toLowerCase()
    );
  }

  function getOrdersWithDeliveredStatus(orders: Order[]) {
    return orders.filter(order => 
  ...
  const handlePickup = async (orderId: string) => {
    markOrderAsOutForDelivery(orderId)
  }
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-muted-foreground">Welcome back, Alex Rodriguez</p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">To Pick Up</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              {orderToPick.length === 0 && (
                <div className="col-span-full text-center py-10">
                  ...
                  <p className="text-muted-foreground">All orders have been picked up.</p>
                </div>
              )}
            </div>
          </TabsContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {orderOutForDelivery
                .map((order) => (
                  <Card key={order.id}>
                    ...
                    <CardFooter>
                      <Button className="w-full" onClick={() => openConfirmDialog(order)}>
                        Mark as Delivered
                      </Button>
                    </CardFooter>
                  </Card>
              ...
              {orderOutForDelivery.length === 0 && (
                <div className="col-span-full text-center py-10">
                  ...
                  <p className="text-muted-foreground">Pick up some orders to deliver.</p>
                </div>
              )}
            </div>
          </TabsContent>
      ...
      <Dialog open={confirmDialogOpen} onOpenChange={setConfirmDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => selectedOrder && handleDeliver(selectedOrder.id)}>Confirm Delivery</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
