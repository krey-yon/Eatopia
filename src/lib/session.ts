...
export async function validateSession (token : string) {
    ...
    if (Date.now() >= findSession.expiresAt.getTime() - 1000 * 60 * 60 * 24 * 15) {
    ...
    return { session : {id : findSession.id, userId : findSession.userId, expiresAt : findSession.expiresAt} , user : findSession.user };
}
